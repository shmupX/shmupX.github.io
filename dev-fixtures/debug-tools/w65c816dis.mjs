...
function decode(o, m, x) {
  ...
  switch (mode) {
    ...
    case IMM8:
      ...
      break;
    case IMM_M:
      arg = m ? `#$${hex(b1)}` : `#$${hex(w, 4)}`;
      break;
    case IMM_X:
      arg = x ? `#$${hex(b1)}` : `#$${hex(w, 4)}`;
      break;
    case DP:
      arg = `$${hex(b1)}`;
      break;
...
if (rest[0] === "--xref") {
  ...
  for (let o = 0; o + 3 < rom.length; o++) {
    ...
    const [name, mode] = e;
    console.log(
      `${("0x" + o.toString(16)).padEnd(9)} ${cpuStr(o)}  ${name} $${
        hex(wantBank)
      }:${hex(wantAddr, 4)}${mode === ABLX ? ",X" : ""}`,
    );
  }
  process.exit(0);
}
  ...
  for (let o = 0; o + 3 < rom.length; o++) {
    ...
    if (op === 0x22 || op === 0x5c) { // JSL / JML long
      ...
      if (rom[o + 3] === wantBank && addr === wantAddr) {
        console.log(
          `${("0x" + o.toString(16)).padEnd(9)} ${cpuStr(o)}  ${
            op === 0x22 ? "JSL" : "JML"
          }`,
        );
      }
...
for (let n = 0; n < count && off < rom.length; n++) {
  ...
  console.log(
    `${("0x" + off.toString(16)).padEnd(8)} ${cpuStr(off)}  ${
      bytes.padEnd(12)
    } ${`m${m}x${x}`}  ${d.text}`,
  );
  off += d.len;
