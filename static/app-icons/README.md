...
`launcher-256.png` as its `.DirIcon`/desktop icon, `cmg.ico` is the Windows
`.exe` icon (`deno task build:windows`), and the macOS `.app`
(`deno task build:mac`) carries an `.icns` the build script assembles out of
`icon-{32,128,256,512}.png`. When the AppImage is added
