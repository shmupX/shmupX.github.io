...
export const SFC_REGIONS = Object.freeze([
    ...
    region("map", "MAP DATA", 0x00340, 0x03940, "likely",
        "6 stages x 0x900 bytes of 8-bit chip indices into MAP GROUP, 18 columns x 128 rows (the width is the vertical-continuity peak in all six sample stages; columns 16-17 are sparse everywhere and may be per-row markers); bit 7 is set on some cells."),
    ...
    region("mapGroup", "MAP GROUP", 0x04540, 0x04B40, "likely",
        "192 chips x 4 SNES tilemap words, free-form tile picks (tiles 0x2C0-0x35F in the sample); 0xFFFF and 0x03FF mean an empty slot, and 119 of the sample's chips are wholly empty."),
    ...
    region("enemyGroup", "ENEMY GROUP", 0x04B80, 0x04E80, "likely",
        "24 enemies x 32 bytes = 4 quads of tilemap words."),
    region("bossGroup", "BOSS GROUP", 0x04E80, 0x05000, "likely",
        "6 bosses x 64 bytes = 8 quads; symmetric strips [n, n+1, n+1|H, n|H] in the sample."),
    ...
    region("reserved1", "RESERVED", 0x07FD2, 0x07FF8, "confirmed", "38 bytes, mostly 0xFF in the sample."),
    ...
    region("myShipGroup", "MY SHIP GROUP", 0x0FF80, 0x10000, "likely", "16 quads of tilemap words (tiles 0x200-0x23C in the sample)."),
