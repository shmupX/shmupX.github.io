...
async function embedArgs(opts: Options): Promise<string[]> {
  ...
  if (opts.exportTools) {
    ...
    // binary — the tool three levels down with the rest of harbor, the game at
    // the root — and lib/export-build.ts stages them back into one tree, which
    // is the layout the tool's own root search expects to find.
    args.push(
      "--include",
...
async function buildIcns(sources: string[]): Promise<Uint8Array> {
  ...
  for (const source of sources) {
    ...
    if (!types) {
      console.warn(
        `  skipping ${source}: no icns type for a ${size ?? "?"}px PNG`,
      );
      continue;
    }
...
async function buildLevel(opts: Options): Promise<void> {
  // harbor's own, not the checkout's — the tool moved down here with the rest
  // of the port layer, while the tree it builds against stayed at ROOT.
  const toolDir = join(harborRoot(), "tools", "build-level");
  const tool = join(toolDir, "index.js");
  ...
  const args = [toolDir, levelName, opts.platform];
...
async function reportArtifacts(
  ...
  const usual = platform === "linux" ? ".appimage" : platform === "windows"
