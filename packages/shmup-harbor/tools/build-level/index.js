...
// The checkout this tool builds out of. It used to be __dirname/../.. — true
// while the tool sat at tools/build-level in the repo root, and wrong once the
// port layer moved into packages/shmup-harbor. It is not a fixed distance
// either way: the packaged desktop app stages this tool and the game side by
// side in a temp dir (lib/export-build.ts stageEmbeddedRuntime), where the
// depth is two again. So walk up for the base game the way lib/repo-root.ts
// does on the Deno side, and let $SHMUPX_ROOT override both.
const CMG_ROOT = cmgRoot();

function cmgRoot() {
  if (process.env.SHMUPX_ROOT) return path.resolve(process.env.SHMUPX_ROOT);
  const marker = path.join("static", "games", "2028-ai", "game.bundle.js");
  let dir = __dirname;
  for (let i = 0; i < 8; i++) {
    try {
      if (fs.statSync(path.join(dir, marker)).isFile()) return dir;
    } catch { /* keep walking */ }
