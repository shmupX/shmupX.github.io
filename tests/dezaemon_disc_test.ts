...
Deno.test({
  ...
  async fn() {
    ...
    // The data track leads either way. A checkout may hold the bare track
    // (the cue is then generated for it alone) or a full dump with its own
    // .cue beside it, which also names the audio track — the disc follows
    // the cue it found, so the file list is whatever that cue names.
    assertEquals(disc.files[0].name, FIXTURE);
    ...
    if (disc.cueFrom === "generated") {
      ...
      assertEquals(disc.files.map((f) => f.name), parseCueFiles(disc.cue));
    }
    const zip = await buildDezaemonDiscZip(disc, { cache: false });
    const entries = await unzip(zip);
    assertEquals(entries.map((e) => e.path), [
      DEZAEMON_CUE_NAME,
      ...disc.files.map((f) => f.name),
    ]);
